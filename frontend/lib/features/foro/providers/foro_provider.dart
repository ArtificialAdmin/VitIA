...
class ForoNotifier extends AsyncNotifier<List<Map<String, dynamic>>> {
  @override
  ...
  Future<List<Map<String, dynamic>>> _fetchPosts() async {
    final foroService = ref.read(foroDataSourceProvider);
    final rawList = await foroService.getPublicaciones();
  ...
  Future<void> toggleLike(int postId) async {
    ...
    try {
      final foroService = ref.read(foroDataSourceProvider);
      if (!currentlyLiked) {
        await foroService.likePublicacion(postId);
      } else {
        await foroService.unlikePublicacion(postId);
      }
    } catch (e) {
  ...
  Future<void> deletePost(int postId) async {
    ...
    try {
      final foroService = ref.read(foroDataSourceProvider);
      await foroService.deletePublicacion(postId);
...
final foroProvider =
    AsyncNotifierProvider<ForoNotifier, List<Map<String, dynamic>>>(
        ForoNotifier.new);

final myPostsProvider = Provider<List<Map<String, dynamic>>>((ref) {
  final posts = ref.watch(foroProvider).valueOrNull ?? [];
...
final foroDataSourceProvider = Provider<ForoService>((ref) => ref.watch(apiProvider).foroDataSource);
