...
class ForoPostDetallePage extends ConsumerStatefulWidget {
  ...
  const ForoPostDetallePage({super.key, required this.post});

  @override
  ConsumerState<ForoPostDetallePage> createState() => _ForoPostDetallePageState();
}

class _ForoPostDetallePageState extends ConsumerState<ForoPostDetallePage> {
  ...
  Future<void> _cargarComentarios() async {
    try {
      ...
      final results = await ref.read(apiProvider).getComentariosPublicacion(id);
      if (mounted) {
        setState(() {
  ...
  Future<void> _publicarComentario() async {
    ...
    try {
      await ref.read(apiProvider).createComentario(
  ...
  Future<void> _borrarPublicacion() async {
    ...
    try {
      await ref.read(foroProvider.notifier).deletePost(widget.post['id']);
      if (mounted) {
  ...
  Future<void> _toggleLikeComentario(int commentId) async {
    if (AuthSessionService.token == null) return;
    ...
    try {
      ...
      if (c['is_liked'] == true) {
        await ref.read(apiProvider).likeComentario(commentId);
      } else {
        await ref.read(apiProvider).unlikeComentario(commentId);
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final forumState = ref.watch(foroProvider);
