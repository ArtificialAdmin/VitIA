...
class ColeccionDetallePage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ColeccionDetallePage> createState() => _ColeccionDetallePageState();
}

class _ColeccionDetallePageState extends ConsumerState<ColeccionDetallePage> {
  ...
  Future<void> _guardarCambios() async {
    ...
    try {
      ...
      await ref.read(apiProvider).updateCollectionItem(_itemActual['id'], updates);
  ...
  Future<void> _confirmarEliminacion() async {
    ...
    if (confirmar == true) {
      try {
        await ref.read(apiProvider).deleteCollectionItem(_itemActual['id']);
        if (mounted) {
          if (widget.onClose != null) {
            widget.onClose!(true);
          } else {
